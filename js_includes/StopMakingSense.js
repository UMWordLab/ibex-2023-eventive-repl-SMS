    ...
    jqueryWidget: {
      _init: function () {
        ...
        if (typeof this.options.s == "string") {
          ...
          assert_is_arraylike(
            this.options.s,
            "Bad value for 's' option of DashedSentence."
          );
        ...
        assert(
          this.display == "dashed" || this.display == "in place",
        ...
        if (this.hideUnderscores) {
          this.words = $.map(this.words, function (word) {
        ...
        if (this.isIE) this.background = "white";
  
        // Defaults.
        this.unshownBorderColor = dget(
          this.options,
          "unshownBorderColor",
          "#9ea4b1"
        );
        this.shownBorderColor = dget(this.options, "shownBorderColor", "black");
        this.unshownWordColor = dget(
          this.options,
          "unshownWordColor",
          this.background
        );
        ...
        assert(
          this.sentenceDescType == "md5" || this.sentenceDescType == "literal",
          "Bad value for 'sentenceDescType' option of DashedSentence."
        );
        if (this.sentenceDescType == "md5") {
          ...
          else this.sentenceDesc = csv_url_encode(this.options.s.join(" "));
        }
        ...
        for (var j = 0; j < this.words.length; ++j) {
          if (this.words[j] == "\r") {
            ...
            if (j <= this.stoppingPoint) this.stoppingPoint--;
          ...
          if (!this.showAhead) ospan.css("border-color", this.background);
          this.mainDiv.append(ospan);
          if (j + 1 < this.words.length) this.mainDiv.append("&nbsp; ");
        ...
        this.safeBind($(document), "keydown", function (e) {
          ...
          if (code == t.options.yesKeyCode) {
            ...
            //
            if (t.currentWord < t.stoppingPoint) t.showWord(t.currentWord);
            ++t.currentWord;
            if (t.currentWord > t.stoppingPoint) {
              t.processSMSResults();
              t.finishedCallback(t.resultsLines);
      ...
      processSMSResults: function () {
        var nonSpaceWords = [];
        for (var i = 0; i < this.words.length; ++i) {
          if (this.words[i] != "\r") nonSpaceWords.push(this.words[i]);
        }
        ...
        for (var i = 0; i < nonSpaceWords.length; ++i) {
          ...
          this.resultsLines.push([
            ...
            [
              "Newline?",
              !this.display == "in place" &&
                boolToInt(
                  i + 1 < this.wordOSpans.length &&
                    this.wordOSpans[i].offset().top !=
                      this.wordOSpans[i + 1].offset().top
                ),
            ],
